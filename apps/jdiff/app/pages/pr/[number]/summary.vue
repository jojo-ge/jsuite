...
// On-demand walkthrough modes (detail tour, system chains) — generated and
// read here just like on the review page; stop links carry ?tour= so the
// review page opens the right walk.
  ...
  <main class="summary-page">
    ...
    <div class="pr-head">
      ...
      <div class="rating-card">
        <div
          class="rating-head"
          :class="{ clickable: modeDetail.tour && !modeDetail.pending }"
          @click="modeDetail.tour && !modeDetail.pending && (detailOpen = !detailOpen)"
        >
          ...
          <template v-if="modeDetail.tour">
            ...
            <span v-if="modeDetail.stale" class="stale-badge" title="the PR was pushed after this tour was written — regenerate to refresh">out of date</span>
          </template>
          <span class="head-actions">
            <button
              v-if="!modeDetail.pending"
              class="log-toggle"
              @click.stop="modes.generate('detail')"
            >{{ modeDetail.tour ? '↻ regenerate' : '✦ generate' }}</button>
        ...
        <template v-if="modeDetail.tour && !modeDetail.pending && detailOpen">
          ...
          <ol class="reading-order">
            <li v-for="(s, i) in modeDetail.tour.stops" :key="i">
              ...
              <div>
                <NuxtLink
                  :to="{ path: reviewRoute.path, query: { repo, stop: i, tour: 'detail' } }"
                  class="reading-path"
                >{{ s.path }}:{{ s.line }}</NuxtLink>
              </div>
              <div class="item-note">{{ s.note }}</div>
      ...
      <div class="rating-card">
        <div
          class="rating-head"
          :class="{ clickable: modeChains.manifest && !modeChains.scopePending }"
          @click="modeChains.manifest && !modeChains.scopePending && (chainsOpen = !chainsOpen)"
        >
          ...
          <template v-if="modeChains.manifest">
            ...
            <span v-if="modeChains.stale" class="stale-badge" title="the PR was pushed after the chains were mapped — re-map to refresh">out of date</span>
          </template>
          <span class="head-actions">
            <button
              v-if="!modeChains.scopePending && !modeChains.anyChainPending"
              class="log-toggle"
              @click.stop="modes.generate('chains')"
            >{{ modeChains.manifest ? '↻ re-map' : '✦ generate' }}</button>
          ...
          <ol class="reading-order">
            <li v-for="c in modeChains.manifest.chains" :key="c.id">
              <strong class="stop-title">
                ...
                >{{ openChains[c.id] ? '▾' : '▸' }} {{ c.title }}</button>
              ...
              <ol v-if="openChains[c.id] && modeChains.chainTours[c.id]" class="reading-order nested">
                <li v-for="(s, i) in modeChains.chainTours[c.id]!.tour.stops" :key="i">
                  ...
                  <div>
                    <NuxtLink
                      :to="{ path: reviewRoute.path, query: { repo, stop: i, tour: `chain:${c.id}` } }"
                      class="reading-path"
                    >{{ s.path }}:{{ s.line }}</NuxtLink>
                  </div>
                  <div class="item-note">{{ s.note }}</div>
