...
- Pool: `.data/jrig/documents/` at the jSuite root (gitignored state — suite convention). First boot seeds `house` + `hoodie` + the 10 built-in clips from the TS sources via `rig/migrate.ts`.
