...
// a clone that moved or a repo nothing points at any more is obvious. Rows
// carry the codebase's settings too: its prompt overrides, its worktree guide
// and links, and the guide's derived `worktree` state.
...
export default defineEventHandler(() => {
  ...
  const repos = store.repos
    .map((r) => ({
      ...
      // The worktree guide's standing, re-derived per read (a changed source
      // file makes it stale) — codebase settings and the switcher show it.
      worktree: guideState(r.path, r.worktreeGuide),
