...
const endpoints = [
  ...
  { m: 'POST', p: '/api/projects/:id/integration-branch', d: 'Cut (or adopt) the empty integration branch { branch?, base?, worktree? } and push it' },
  { m: 'POST', p: '/api/projects/:id/worktree', d: 'Give the project a checkout of its integration branch under .worktrees/ { slug?, boot? } — fleet slot and stack where the repo has one' },
  { m: 'DELETE', p: '/api/projects/:id/worktree', d: 'Tear that worktree down (?force=1 when dirty, ?forget=1 to drop only the record)' },
  { m: 'POST', p: '/api/projects/:id/sync', d: 'Push the integration branch to origin — the manual retry; merges push it themselves once the roll-up PR exists' },
