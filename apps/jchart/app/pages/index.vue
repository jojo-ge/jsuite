...
async function create() {
  ...
  creating.value = true
  try {
    const res = await $fetch<{ path: string }>('/api/charts', {
      method: 'POST',
...
<template>
  <div class="mx-auto min-h-screen w-full max-w-3xl px-6 py-10">
    <div class="mb-8 flex items-end gap-4">
      <div class="flex-1">
        <h1 class="text-2xl font-semibold tracking-tight">jChart</h1>
        <p class="text-sm text-muted">Editable, annotatable diagrams. Claude drafts them; you redraw and mark them up.</p>
      </div>
    ...
    <ul v-else class="space-y-2">
      ...
      >
        <NuxtLink :to="`/c/${c.key}`" class="min-w-0 flex-1">
