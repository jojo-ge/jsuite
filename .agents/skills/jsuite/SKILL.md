...
no diffs — jDiff renders those; only the integration branch ever reaches origin,
and it pushes itself on every merge once the roll-up PR exists). A project can
also own a **worktree** — its integration branch checked out at
`<repo>/.worktrees/<slug>`, claimed as a `./kraken wt` fleet slot where the repo
has one — which every merge fast-forwards, and where the project's herdr agents
run. It can also **sync a project between two machines** (jTicket
