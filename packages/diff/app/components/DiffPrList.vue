...
const { isRunning, startAll, cancel, running } = useDiffAiTasksHub(repo)
...
function jobLink(job: { id: string }) {
  return job.id.startsWith('branch/')
    ? routes.branch({ repo: repo.value, branch: job.id.slice('branch/'.length) })
    : routes.pr(repo.value, job.id)
}
...
<template>
  <main class="diff-surface prs">
    <header class="bar">
      ...
      <DiffNotificationBell :repo="repo" />
    </header>
    ...
    <div v-if="authors.length > 1" class="people">
      ...
      >
        <DiffAuthorAvatar :login="a.login" :size="26" />
    ...
    <ul v-else class="list">
      <template v-for="(pr, i) in filtered" :key="pr.number">
        ...
        >not analyzed yet</li>
        <li>
          <NuxtLink :to="routes.pr(repo, pr.number)" class="pr">
            <div class="top">
            ...
            <div class="meta">
              <span class="who">
                <DiffAuthorAvatar :login="pr.author?.login" :size="16" />
                {{ pr.author?.login }}
              </span>
