...
| multi-reviewer code review — several reviewers, dedupe, findings → tickets | `apps/jreview` | the New review button on :43008 dispatches four Opus 5.5 reviewers into herdr (always the global `/code-review` skill), each publishing a jExplain report at a pre-assigned doc key via `/jreview-report`; the server-side watcher (`server/plugins/reviewWatcher.ts`) sees every report land and dispatches `/jreview-triage`, which POSTs deduped findings back; the human's button splits them into a new jTicket project. State in `.data/jreview/` |
| run a project hands-off — implement → merge → review → fix loops | `apps/jticket` | the project's **jButton** (auto mode, confirm modal; `POST /api/projects/:id/auto`) — `server/plugins/autoLoop.ts` dispatches the AFK frontier on Opus 5.5 (`/jimplement`, local PRs), waits for every ticket, runs the merge sweep on Sonnet 5, asks jReview for a 2-reviewer consensus review of the loop's diff (`reviewers: 2, consensus: {projectKey, loop}`; its Sonnet 5 `/jreview-consensus` session files only findings BOTH reviewers raised as tickets in the project), implements + merges those, then starts the next loop. HITL tickets are never auto-dispatched. "Stop at the end of next loop" finishes the loop in progress, then turns auto off. State: `project.auto` |
