...
| PR review, branch diff, review comments, staged/unstaged changes | `apps/jdiff` | `jdiff` CLI (`jdiff pr N`, `jdiff branch B`, `jdiff branch -s staged\|unstaged\|everything`, `--print`); review pages add the detail / chains / hunt walkthroughs — hunt reviews the change for bugs and vulnerabilities and walks every HIGH one in depth |
...
| dispatching terminal claude sessions into herdr | shared pool | `packages/herdr` (`ensureHerdrWorkspace`, `acquirePackedPane`, `startClaudeIn`) — jTicket, jMap and jDiff all dispatch through it (jDiff's review sessions run the `jdiff-review`/`jdiff-ask`/`jdiff-tour`/`jdiff-chains`/`jdiff-hunt` skills on Opus 5); jTicket's Run-review buttons proxy to jDiff's dispatch with ticket/project context, and the review session reports findings back into jTicket (fix tickets / ticket comments) |
