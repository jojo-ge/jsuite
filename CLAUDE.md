...
| block documents (docs, specs, explainers) | shared pool | `packages/documents` serves `/api/documents` **and a whole-pool library at `/documents`** (reader at `/documents/<key>`) over `.data/jexplain/` in jTicket, jExplain AND jGrilling — a jTicket doc IS a jExplain document. Every consumer shadows those two routes with its own: jTicket groups the list by project, jGrilling withholds delete and funnels its reader to `/e/<key>`, jExplain redirects both onto its `/` and `/e/<key>`. The components take a `deletable` prop that **defaults off** — see "who may delete out of the pool" in `README.md` before mounting either |
